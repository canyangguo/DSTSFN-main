...
from .metrics import masked_mape_np, masked_smape_np, masked_wmape_np
from scipy.sparse.linalg import eigs
from .metrics import masked_mape_np,  masked_mae,masked_mse,masked_rmse,masked_mae_test,masked_rmse_test, masked_msis_np
    ...
    with torch.no_grad():
        ...
        for i in range(prediction_length):
            ...
            if metric_method == 'mask':
                ...
                smape = masked_smape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
                wmape = masked_wmape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
                msis = masked_msis_np(data_target_tensor[:, :, i:i+1].transpose(0,2,1), prediction[:, :, i:i+1].transpose(0,2,1), 0, i)
            else :
            ...
            print('all msis: %.2f' % (msis))
            excel_list.extend([mae, rmse, mape, smape, wmape, msis])
        ...
        if metric_method == 'mask':
            ...
            smape = masked_smape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
            wmape = masked_wmape_np(data_target_tensor[:, :, i], prediction[:, :, i], 0)
            msis = masked_msis_np(data_target_tensor.transpose(0,2,1), prediction.transpose(0,2,1), 0)
        else :
        ...
        print('all msis: %.2f' % (msis))
        excel_list.extend([mae, rmse, mape, smape, wmape, msis])
        for i in range(len(excel_list)//6):
            print('@t' + str(i+1) + ' {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}'.format(*excel_list[i*6: (i+1)*6]))
