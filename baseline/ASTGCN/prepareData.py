...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    data_seq = data_seq[:, :, 0: 1]
    index = np.arange(data_seq.shape[0]).reshape(data_seq.shape[0], 1, 1).repeat(data_seq.shape[1], axis=1)
    
    if len(data) % 288 == 0:
        print(len(data_seq) / 288)
        time_index = (index % 288) / 288
        day_index = (index // 288) % 7
    else:
        print(len(data_seq) / 144)
        time_index = (index % 144) / 144
        day_index = (index // 144) % 7


    data_seq = np.concatenate([data_seq, time_index, day_index], axis=-1)
...
def normalization(train, val, test):
    ...
    mean = train[..., 0:1, :].mean(axis=(0,1,3), keepdims=True)
    std = train[..., 0:1, :].std(axis=(0,1,3), keepdims=True)
