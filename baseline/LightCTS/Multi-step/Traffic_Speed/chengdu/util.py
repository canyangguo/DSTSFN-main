...
class StandardScaler():
    ...
    def transform(self, data):
        data[..., 0:1] = (data[..., 0:1] - self.mean) / self.std
        return data

    def inverse_transform(self, data):
        data[:, 0:1] = (data[:, 0:1] * self.std) + self.mean
        return data
...
def metric(pred, real):
    ...
    msis = masked_msis(pred.transpose(1,-1),real.transpose(1,-1),0.0).item()


    return mae,mape,rmse,smape,wmape, msis
'''
...
def generate_from_data(origin_data, length, transformer): 
    ...
    index = np.arange(origin_data.shape[0]).reshape(origin_data.shape[0], 1, 1).repeat(origin_data.shape[1], axis=1)
    
    if len(origin_data) % 288 == 0:
        print(len(origin_data) / 288)
        time_index = (index % 288) / 288
        day_index = (index // 288) % 7
    else:
        print(len(origin_data) / 144)
        time_index = (index % 144) / 144
        day_index = (index // 144) % 7


    origin_data = np.concatenate([origin_data, time_index, day_index], axis=-1)

    train_line, val_line = int(length * 0.6), int(length * 0.8)
    for key, line1, line2 in (('train', 0, train_line),#(('train', 0, 3*288),#train_line),
                              ('val', train_line, val_line),
                              ('test', val_line, length)):

        x, y = generate_seq(origin_data[line1: line2], 12, 12)
        data['x_' + key] = x.astype('float32')
        data['y_' + key] = y[..., 0: 1].astype('float32')
...
def generate_data(graph_signal_matrix_name, batch_size, test_batch_size=None, transformer=None):
    ...
    data = generate_from_data(origin_data, length, transformer)
    scalar = StandardScaler(mean=data['x_train'][..., 0:1].mean(), std=data['x_train'][..., 0:1].std())
    for category in ['train', 'val', 'test']:
...
def generate_from_data(origin_data, length, transformer):#main
    data = {}
    
    origin_data = (origin_data)[:, :, 0: 1]
    index = np.arange(origin_data.shape[0]).reshape(origin_data.shape[0], 1, 1).repeat(origin_data.shape[1], axis=1)
...
def generate_seq(data, train_length, pred_length): #main
    seq = np.concatenate([np.expand_dims(
        data[i: i + train_length + pred_length], 0)
        for i in range(data.shape[0] - train_length - pred_length + 1)],
        axis=0)#[:, :, :, 0: 1]
