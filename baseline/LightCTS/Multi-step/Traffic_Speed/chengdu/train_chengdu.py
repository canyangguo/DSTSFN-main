...
def main():
    ...
    adj_mx = np.load('data/chengdu/matrix.npy')#get_adj_matrix('data/chengdu/matrix.csv', 524)
    dataloader = generate_data('data/chengdu/dataset.npy', args.batch_size, args.batch_size)
    ...
    print('donot load best val model')
    ...
    for i in range(12):
        ...
        log = 'Evaluate best model on test data for horizon {:d}, Test MAE: {:.4f}, Test MAPE: {:.4f}, Test RMSE: {:.4f}, Test SMAPE: {:.4f}, Test WMAPE: {:.4f}'
        print(log.format(i+1, metrics[0], metrics[1], metrics[2], metrics[3], metrics[4]))
        amae.append(metrics[0])
        amape.append(metrics[1])
        armse.append(metrics[2])
        smape.append(metrics[3])
        wmape.append(metrics[4])

    log = 'On average over 12 horizons, Test MAE: {:.4f}, Test MAPE: {:.4f}, Test RMSE: {:.4f}, Test SMAPE: {:.4f}, Test WMAPE: {:.4f}'
