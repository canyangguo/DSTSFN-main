...
def normalize_dataset(data, normalizer, column_wise=False):
    ...
    if normalizer == 'max01':
        ...
        if column_wise:
            mean = data[:, :, 0:1].mean(axis=0, keepdims=True)
            std = data[:, :, 0:1].std(axis=0, keepdims=True)
        else:
            mean = data[:, :, 0:1].mean()
            std = data[:, :, 0:1].std()
        ...
        raise ValueError
    print('normalize finished***********', normalizer)
...
def get_dataloader_cde(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    ...
    if args.test_ratio > 1:
        ...
        data_train, data_val, data_test = split_data_by_ratio(data, args.val_ratio, args.test_ratio)
    #add time window
    _, y_test = Add_Window_Horizon(data_test, args.lag, args.horizon, single)
    ...
    x_tra, y_tra = Add_Window_Horizon(data_train, args.lag, args.horizon, single)
    x_val, y_val = Add_Window_Horizon(data_val, args.lag, args.horizon, single)
    x_test, _ = Add_Window_Horizon(data_test, args.lag, args.horizon, single)
