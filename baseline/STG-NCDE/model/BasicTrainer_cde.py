...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        print("an epoch time: ", time.time()-bt)
    ...
    def test(model, args, data_loader, scaler, logger, path, times):
        ...
        for t in range(y_true.shape[1]):
            mae, rmse, mape, smape, wmape, msis = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh, t)
            logger.info("Horizon {:02d} {:.2f} {:.2f} {:.4f} {:.4f} {:.4f} {:.4f}".format(
                t + 1, mae, rmse, mape*100, smape*100, wmape*100, msis))
        mae, rmse, mape, smape, wmape, msis = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh)
        logger.info("Horizon Average {:.2f} {:.2f} {:.4f} {:.4f} {:.4f} {:.4f}".format(
                    mae, rmse, mape*100, smape*100, wmape*100, msis))
    ...
    def test_simple(model, args, data_loader, scaler, logger, path, times):
        ...
        for t in range(y_true.shape[1]):
            mae, rmse, mape, smape, wmape = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh)
        mae, rmse, mape, smape, wmape = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh)
        logger.info("Average Horizon, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}, SMAPE: {:.4f}, WMAPE: {:.4f}%".format(
                    mae, rmse, mape*100, smape*100, wmape*100))
