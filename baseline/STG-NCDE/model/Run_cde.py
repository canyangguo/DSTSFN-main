...
Mode = 'train'
DEBUG = 'False'
DATASET = 'shenzhen'      #PEMSD4 or PEMSD8
MODEL = 'GCDE'
...
args.add_argument('--input_dim', default=4, type=int) #default=config['model']['input_dim'], type=int)
...
print('the number of parameters:{}'.format(total))
...
if args.mode == 'train':
    ...
    path = '../runs/PEMSD8/12-12-13h13m_PEMSD8_GCDE_type1_embed{10}hid{64}hidhid{64}lyrs{2}lr{0.001}wd{0.001}/best_model.pth'

    model.load_state_dict(torch.load(path))
    print("Load saved model")
    trainer.test(model, trainer.args, test_loader, scaler, trainer.logger, path, times)
else:
    raise ValueError
