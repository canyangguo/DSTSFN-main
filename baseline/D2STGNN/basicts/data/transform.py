...
# 修改了函数名来调整mean-std 和max-min,生成数据也要用到这个，所以要重新生成数据
...
def standard_transform(data: np.array, output_dir: str, train_index: list, history_seq_len: int, future_seq_len: int) -> np.array:
    ...
    #scaler["func1"] = standard_transform.__name__
    #scaler["args1"] = {"mean": mean, "std": std}
...
@SCALER_REGISTRY.register()
def standard_transform_(data: np.array, output_dir: str, train_index: list, history_seq_len: int, future_seq_len: int) -> np.array:
...
@SCALER_REGISTRY.register()
def re_standard_transform_(data: torch.Tensor, **kwargs) -> torch.Tensor:
