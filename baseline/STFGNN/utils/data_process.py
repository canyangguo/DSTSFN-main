...
def load_data(graph_signal_matrix_filename, batch_size, test, log, data_name):
...
def load_adj(adj_dtw_filename, graph_signal_matrix_filename, adj_filename, num_of_vertices, id_filename, model_name, data_name):
    ...
    if graph_signal_matrix_filename[:9] == 'data/PEMS':
    ...
    #dtw_matrix = generate_dtw_matrix(train_data, data_name)  

    

    #np.savetxt("adj_shenzhen_001.csv", dtw_matrix, delimiter = ",")
    ...
    if data_name == 'PEMS':
        ...
        adj = get_adjacency_matrix(adj_filename, num_of_vertices, id_filename=id_filename)  # spatial correlation
    ...
    adj_mx = construct_adj_fusion(adj, 0, adj_dtw, 4, model_name)
    ...
    return adj_st, mask_init_value_st
...
def generate_dtw_matrix(data, data_name):

    node = data.shape[1]

    dtw = np.zeros((node, node))

    for i in range(0, node-1):

        for j in range(i, node):

            dtw[i][j] = compute_dtw(data[:, i], data[:, j], data_name)
...
def compute_dtw(a, b, data_name, o=1, T=12):
...
def construct_adj_fusion(adj_s, adj_gm, adj_dtw, steps, type='STSGCN'):
    ...
    adj = np.r_[
        ...
        np.c_[adj_dtw, adj_T, adj_T, adj_dtw],



        np.c_[adj_T, adj_s, adj_T, adj_T],



        np.c_[adj_T, adj_T, adj_s, adj_T],
