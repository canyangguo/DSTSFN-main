...
class stack_gcn(nn.Module):
    ...
    def forward(self, data, adj):
        ...
        for i, gcn in enumerate(self.gcn_operations):  # multi filters
            ...
            # (1, N, B, C')
            need_concat.append(data[-self.num_of_vertices:].unsqueeze(0))

        # (3, N, B, C'), 3 denote number of gcn filters

        need_concat = torch.cat(need_concat, dim=0)
...
class fstgcn(nn.Module):
    ...
    def forward(self, original_data, data, adj_st):
        ...
        data_time_axis = torch.sigmoid(data_left) * torch.tanh(data_right)
...
class s2t(nn.Module):
    def __init__(self, num_of_features, input_length, num_of_vertices):
        ...
        self.fc1 = fc(num_of_features, num_of_features)
        self.fc2 = fc(num_of_features, num_of_features)
    ...
    def forward(self, data, temp, adj_s, adj_t):
        res = self.fc1(data) + self.fc2(temp)
        data = self.position_embedding(data)
...
class _main(nn.Module):
    def __init__(self,
        ...
        self.fc = fc(3, first_layer_embedding_size)  #shenzhen 2
    ...
    def forward(self, data):
        ...
        # (B, T, N, C) = (B, T, N, F) * (F, C)
        data = self.fc(data[...,:3])  # first layer embedding #shenzhen 2
