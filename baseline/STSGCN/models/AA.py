...
class mf(nn.Module):
    def __init__(self, merge_step, num_of_vertices, num_of_latents, dropout_rate):
...
class s2t(nn.Module):
    def __init__(self, num_of_features, input_length, num_of_vertices):
        ...
        self.num_of_features = num_of_features
        self.weight = nn.Parameter(Tensor(num_of_features, num_of_features*2))
        self.bias = nn.Parameter(Tensor(1, num_of_features*2))
    ...
    def forward(self, data, temp, adj_s, adj_t):
        ...
        # split(N, B, 2C') = (N, B, C'), (N, B, C')
        lhs, rhs = torch.split(data, self.num_of_features, dim=3)
...
class _main(nn.Module):
    def __init__(self,
        ...
        self.num_of_latents = 32
        #self.dropout_rate = 0.90, 0.8, 0.7, 0.6, 0.5, 0.2
        self.dropout_rate = 0.90
        print('dropout_rate =', self.dropout_rate)
        print('num_of_latents =', self.num_of_latents)

        self.mf = mf(merge_step, num_of_vertices, self.num_of_latents, self.dropout_rate)

        self.mf_s2 = mf(1, num_of_vertices, self.num_of_latents, self.dropout_rate)
        self.mf_s3 = mf(1, num_of_vertices, self.num_of_latents, self.dropout_rate)

        self.mf_t2 = nn.Parameter(Tensor(12, 12))
        self.mf_t3 = nn.Parameter(Tensor(12, 12))
        self.I = torch.eye(12).cuda()
        ...
        self.conv2 = nn.Conv2d(in_channels=self.num_of_features,
                               out_channels=self.num_of_features,
                               kernel_size=(1, 10))
        self.conv3 = nn.Conv2d(in_channels=self.num_of_features,
                               out_channels=self.num_of_features,
                               kernel_size=(1, 10))
