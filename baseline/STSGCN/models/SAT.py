...
class _main(nn.Module):
    def __init__(self,
        ...
        self.weight2 = nn.Parameter(Tensor(num_of_features, first_layer_embedding_size))
        self.bias2 = nn.Parameter(Tensor(1, first_layer_embedding_size))
        ...
        TI = []
        for i in range(receptive_length):
            TI.append(torch.eye(num_of_vertices))
        self.TI = torch.stack(TI, dim=1).reshape(num_of_vertices, -1).cuda()
    ...
    def forward(self, data, x_day_index, x_time_index, y_day_index, y_time_index):
        ...
        # (B, T, N, C) = (B, T, N, F) * (F, C)
        Q = torch.matmul(data, self.weight) + self.bias  # first layer embedding
        data = torch.matmul(data, self.weight2) + self.bias2
        adjs = []

        for i in range(self.receptive_length-1, -1, -1):
            score = torch.einsum('btnc, btmc->nm', Q[:, i:], Q[:, :12-i]) / (self.num_of_features ** 0.5)
            
            adjs.append(score)
        adj = torch.stack(adjs, dim=2).reshape(self.num_of_vertices, self.receptive_length*self.num_of_vertices)
        adj = self.softmax(adj) + self.TI


        adj_s2 = self.softmax(adjs[-1]) + self.TI[:, -self.num_of_vertices]

        adj_t2 = self.softmax(torch.einsum('btnc, bpnc -> tp', Q, Q)) + self.I
        ...
        for i in range(self.stack_times):
            ...
            x3 = self.t2s[i](x3, temp, adj_s2, adj_t2)
        x2 = self.conv2(x2)
        x3 = self.conv3(x3)
        '''
        for i in range(self.stack_times):
