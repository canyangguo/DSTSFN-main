...
class mf(nn.Module):
    def __init__(self, merge_step, num_of_vertices, num_of_latents, dropout_rate, adj_st=None, cand=False):
...
class temporal_adj(nn.Module):
    def __init__(self, t):
        super(temporal_adj, self).__init__()
        self.adj = nn.Parameter(Tensor(t, t))
        self.I = torch.eye(t).cuda()
        self.reset_parameters()

    def reset_parameters(self) -> None:
        torch.nn.init.xavier_normal_(self.adj, gain=0.02)

    def forward(self):
        return self.adj + self.I
...
class _main(nn.Module):
    def __init__(self,
        ...
        self.k = 0
        self.fc = fc(num_of_features, first_layer_embedding_size)
        self.fcs = nn.ModuleList([
                    fc(first_layer_embedding_size, first_layer_embedding_size)
                    for i in range(2*receptive_length)
        ])
        ...
        self.adj_ss = nn.ModuleList([
            mf(1, num_of_vertices, num_of_latents, 1-(1-ST_adj_dropout_rate)*(i+1))
            for i in range(self.k, receptive_length)
        ])

        self.adj_ts = nn.ModuleList([
            temporal_adj(input_length)
            for i in range(self.k, receptive_length)
        ])
        self.masks = []
        for i in range(self.k, receptive_length): # mask
            self.I = torch.zeros(input_length, input_length)
            for j in range(input_length):
                if j < (i+1)*3-(i+2):
                    for k in range(0, (i+1)*3):
                        self.I[j][k] = 1
                elif j+int(((i+1)*3-1)/2) >= input_length:
                    for k in range(input_length-(i+1)*3, input_length):
                        self.I[j][k] = 1
                else:
                    for k in range(j-int(((i+1)*3)/2), j-int(((i+1)*3)/2)+(i+1)*3):
                        self.I[j][k] = 1
            self.masks.append(self.I.cuda())
    ...
    def forward(self, data):
        ...
        datas = data + torch.zeros(data.shape).cuda()

        for i in range(self.receptive_length-self.k):
            datas += self.fcs[i](torch.einsum('ss, btnc -> btnc', self.adj_ss[i](), data))
            datas += self.fcs[i+(self.receptive_length-self.k)](torch.einsum('tt, btnc -> btnc', self.adj_ts[i]()*self.masks[i], data))
        ...
        temp = data
        for i in range(self.stack_times):
            data = self.fstgcns[i](temp, data, adj_st)
