...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        info = []
        for idx in range(12):
            y, x = trues[:, idx: idx + 1, :], preds[:, idx: idx + 1, :]
            info.append(metric(x, y))

        info.append(metric(preds, trues))
        ...
        for i in info:
            if horizon <= 12:
                print('@t' + str(horizon), *i)
            else:
                print('average', *i)
            horizon = horizon + 1
        mae, mse, rmse, mape, mspe, msis = metric(preds, trues)
