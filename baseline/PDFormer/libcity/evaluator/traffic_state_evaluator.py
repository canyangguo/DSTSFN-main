...
class TrafficStateEvaluator(AbstractEvaluator):
    ...
    def __init__(self, config):
        ...
        self.allowed_metrics = ['MAE', 'MSE', 'RMSE', 'MAPE', 'SMAPE', 'WMAPE', 'msis', 'masked_MAE',
                                'masked_MSE', 'masked_RMSE', 'masked_MAPE', 'masked_SMAPE', 
                                'masked_WMAPE', 'masked_msis', 'R2', 'EVAR']
                ...
                for metric in self.metrics:
                    if metric == 'masked_MAE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mae_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'masked_MSE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mse_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'masked_RMSE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_rmse_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'masked_MAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mape_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'masked_SMAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_smape_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'masked_WMAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_wmape_torch(y_pred[:, i-1:i], y_true[:, i-1:i], 0).item())
                    elif metric == 'MAE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mae_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'MSE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mse_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'RMSE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_rmse_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'MAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_mape_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'SMAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_smape_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'WMAPE':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_wmape_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'R2':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.r2_score_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'EVAR':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.explained_variance_score_torch(y_pred[:, i-1:i], y_true[:, i-1:i]).item())
                    elif metric == 'masked_msis':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.masked_msis_torch(y_pred[:, i-1:i], y_true[:, i-1:i], i).item())
                    elif metric == 'msis':
                        self.intermediate_result[metric + '@' + str(i)].append(
                            loss.msis_torch(y_pred[:, i-1:i], y_true[:, i-1:i], i).item())
