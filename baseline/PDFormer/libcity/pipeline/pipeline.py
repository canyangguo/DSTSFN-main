...
def run_model(config, stage, task=None, model_name=None, dataset_name=None, source=None, config_file=None,
    ...
    train_data, valid_data, test_data = dataset.get_data(stage, config)
    ...
    '''
        ...
        model_cache_file = './libcity/cache/{}/model_cache/{}_{}.m'.format(exp_id, model_name, source)
        
        executor.load_model(model_cache_file)
        executor.evaluate(test_data)
        print('\n*****start target stage*****\n')
        executor.train(train_data, valid_data)
        model_cache_file = './libcity/cache/{}/model_cache/{}_{}_ft.m'.format(exp_id, model_name, source)
        executor.save_model(model_cache_file)    
        executor.load_model(model_cache_file)
        executor.evaluate(test_data)
    '''
